...
public class LeadBusiness(MongoDBService mongoDBService, ILogger<LeadBusiness> logger) : ILeadBusiness
{
    ...
    public async Task<List<Dictionary<string, object>>> ProcessFileOld(Stream stream)
    {
    ...
    {
        var resp = await GetFullProspects();

        var report = new Report()
        {
            TotalProspectContacted = resp.Sum(p => p.Interactions.Count > 0 ? 1 : 0),
            TotalProspectContactedTrue = resp.Sum(p => p.Interactions.Any(i => i.Answer) ? 1 : 0),
            TotalInteractions = resp.Sum(p => p.Interactions.Count),
            TotalProspects = resp.Count(),
            TotalInteractionsByEmail = resp.Sum(p => p.Interactions.Count(i => i.Type == InteractionType.Email.ToString())),
            TotalInteractionsByEmailTrue = resp.Sum(p => p.Interactions.Count(i => i.Type == InteractionType.Email.ToString() && i.Answer)),
            TotalInteractionsByEmailFalse = resp.Sum(p => p.Interactions.Count(i => i.Type == InteractionType.Email.ToString() && !i.Answer)),
            TotalInteractionsByPhone = resp.Sum(p => p.Interactions.Count(i => i.Type == InteractionType.Call.ToString())),
            TotalInteractionsByPhoneTrue = resp.Sum(p => p.Interactions.Count(i => i.Type == InteractionType.Call.ToString() && i.Answer)),
            TotalInteractionsByPhoneFalse = resp.Sum(p => p.Interactions.Count(i => i.Type == InteractionType.Call.ToString() && !i.Answer)),
        };
        return report;
    }
    ...
    {
        return await _mongoService.UpdateFullProspect(fullProspect);
    }
}
